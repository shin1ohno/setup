...
- `compose_service "<name>" do … end` - docker-compose deploy pair: `ensure <name> running` (idempotency-probed `up -d`) + notify-driven `restart <name>` (`up -d --force-recreate`). Params: `compose_path`, `deploy_dir`, `env_path`, `buildkit`, `build_flag`, `wait`. Reference adopters: lxc-memory / lxc-roon-mcp / local-mcp / lxc-weave / lxc-consent
