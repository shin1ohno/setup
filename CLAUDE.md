...
- **Guardrails run in CI** (`syntax-check` job): `bin/audit-cookbook-reachability` (no dead/unreachable cookbook; ALLOWLIST is empty) + `bin/lint-cookbooks` (8 checks: compile-time `File.exist?`, Integer owner/group, auth-gate profile mismatch, notify-driven compose without `--force-recreate`, fully-bare auth gate on a fleet cookbook, claude-code deploy-list drift, `owner` without `group`, mruby-absent CRuby methods in recipe Ruby). The linter's own header comment is the canonical list — read it there rather than trusting this summary. Run both before committing a cookbook change.
