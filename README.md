...
**Use `bin/converge <entry.rb>` on a fresh machine.** It runs `bin/setup`, applies `bootstrap.rb` (the prerequisite tools the auth gates test for at compile time — today `aws`), runs `bin/doctor`, and only then applies the entry recipe in a fresh mitamae process, so every SSM-gated cookbook sees its tool and evaluates for real. Exit 3 means INCOMPLETE: a required gate was still skipped (or doctor FAILed) — the message names the fix. Optional features you decline at a prompt are listed as skipped and do not make the run incomplete. Background: `require_external_auth` probes `aws` while the recipe compiles, before the resource that installs it converges, which is why a bare `./bin/mitamae local <entry>.rb` on a fresh machine used to need a second run (ADR 0012).
