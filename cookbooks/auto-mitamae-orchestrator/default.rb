...
WATCHDOG_BIN                  = "/usr/local/bin/orchestrator-watchdog.sh"
...
%w[orchestrator.sh drift-checker.sh orchestrator-watchdog.sh].each do |script|
...
execute "install orchestrator-watchdog.sh to #{WATCHDOG_BIN}" do
  command "sudo install -m 0755 -o root -g root " \
          "#{node[:setup][:root]}/auto-mitamae-orchestrator/orchestrator-watchdog.sh #{WATCHDOG_BIN}"
  not_if "diff -q #{node[:setup][:root]}/auto-mitamae-orchestrator/orchestrator-watchdog.sh #{WATCHDOG_BIN} 2>/dev/null"
end
...
# cron — drift-checker every 2 min, orchestrator every 5 min, watchdog
# every 3 min. Output redirected to a single log file; systemd-cron /
# cronie's default MAILTO=root would otherwise fill /var/spool/mail.
#
...
#
# 2026-05-17 origin: orchestrator process hung 16 min on AWS STS calls
# (DNS AAAA timeout) before manual `kill -TERM` was required. The 5-min
# cron interval then could not start a fresh cycle (flock still held).
cron_content = <<~CRON
