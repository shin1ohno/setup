...
- **Each medium has its own shape**: a channel post puts only the conclusion in the parent message and hangs the evidence, the detail and the options off it as replies. A document gets the prose discipline above unchanged. When the medium itself is ambiguous, that is an AskUserQuestion, not a guess. Origin: recurred in two sessions on 2026-09-10 (「親メッセージには結論を、その他の情報はレスとしてつけて」).
