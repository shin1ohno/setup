...
**AskUserQuestionを使わなくてよい場合**: 指示が明確で、実装方法が1通りしかなく、可逆的な操作のみの場合。
...
- Background first: launch independent research tasks with `run_in_background: true` to minimize user wait time

### Bulk Web Research Pattern

When collecting information from multiple URLs, products, or categories for Cognee ingestion, **proactively** apply this pattern (propose and execute before the user asks for parallelism):

1. **Split by category**: divide targets by category, brand, or theme — 1 agent = 1 category
...
4. **Progress reporting**: update a progress table for the user as each agent completes
...
| Bulk web research (5+ URLs) | Background sub-agents (parallel) |
