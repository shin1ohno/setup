...
- **媒体ごとに型が違う**: チャンネルへ投稿するときは親メッセージに結論だけを置き、根拠・詳細・選択肢はスレッドの返信に付ける。文書として渡すときは上の文書規律をそのまま適用する。媒体の選択自体が曖昧なら推測で決めず AskUserQuestion の対象。Origin: 2026-09-10 に 2 セッションで反復（「親メッセージには結論を、その他の情報はレスとしてつけて」）。
...
- **Self-review pass before presenting a multi-line Plan / report** (Plan, analysis, retro, research summary — *whether or not* it is written to `.md`): the bullets above are "while writing"; this is a mandatory pass over the finished draft *before* it reaches the user. Not optional polish — apply the discipline in full, no half-measures:
  ...
  **発火の線はファイル出力の有無**: ファイルに書き出す成果物（レポート・分析・提案・仕様）は `/writing` skill を spawn して通す。chat に収まる inline report は spawn せず self-apply し、`Read` `~/.claude/skills/writing/references/phrases.md` + `structures.md` で全リストに当てる（記憶から照合しない）。この線は「30 行超・再利用される成果物はファイルに書く」ルールと一致するので、都度の判断が要らない。Single-line factual answers are exempt. Origin: issue #640（inline は self-apply へ）; 2026-09-10 に 3 セッションで「文章としてとても読みにくいですね。/writing skill は適用されていますか？」と指摘され、ファイル出力物は spawn に戻した。
