...
**Expected tool/skill absent → don't silently substitute**: when a tool/skill the user asked for (or the task expects) is confirmed absent via `ToolSearch`, state the absence in one line and offer a fallback via AskUserQuestion (degraded alternative with its limits spelled out, or re-enable and do the real thing) — silent degraded substitution is the same violation as opaque substitution. If AskUserQuestion *itself* is absent, present numbered options at the end of a normal reply and stop (this fallback alone lifts the prose-menu ban) — never bundle undecided items into ExitPlanMode approval. Origin: 2026-06 sage — `open`-url substituted without consent; 5 decisions bundled into a plan approval → 2 rejects.
...
**Negative search is not evidence of absence**: 「参照ゼロ / 存在しない / 未対応」と断定する前に、(a) positive control — 同じ検索コマンドが既知トークンで非ゼロを返すことを確認する（rg 不在・sandbox 遮断・パス誤り・zsh エラーは真の 0 件と出力上区別できない）、(b) `git grep` / `git ls-files` でクロスチェック、(c) 探索でも先頭 `cd` 禁止（chpwd フックの stdout 汚染が偽陰性を生む — `git-commit.md` の git 版ルールの一般化）— 絶対パス引数で。テンプレート機構の漏れ・GitHub 検索の hyphen 分割・兄弟リポ probe の各論: Detail: see `~/.claude/docs/negative-search-detail.md`。Origin: 2026-06-27〜07-03 に 3 プロジェクトで 3 件（「sage 参照ゼロ」直後に servers.yml で発見 / #45 が `--search` 不一致 / cd の tree フック + 浅い find で cookbook 見落とし）。
