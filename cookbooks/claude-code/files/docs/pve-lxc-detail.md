...
Load when a task involves PVE LXC operations (Docker-in-LXC design gate, unprivileged bind-mount UID mapping, `pct exec` TTY / login-shell semantics, privileged-vs-unprivileged systemd hardening). Reached via the CLAUDE.md Detail-playbooks table; cookbook comments across the LXC fleet cite this file's section headings by name.
...
## unprivileged-bind-mount-ownership
...
## pve-lxc-reachability-devices-json
