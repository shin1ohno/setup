...
- **`Regexp.last_match` takes NO arguments** — `Regexp.last_match(1)` raises `wrong number of arguments (1 for 0) (ArgumentError)`, an *arity* error rather than the `NoMethodError` the other cases give. Read captures through `$1` / `$2` (`cookbooks/functions/default.rb`, `_pinned_profile` and `_auth_diagnosis`). Reference the enclosing METHOD, not a line number: the first version of this pointer cited lines computed against the pre-insertion file and was already stale in the commit that added it.
...
CI syntax-check jobs run CRuby (`ruby -c`), which has all of these. A cookbook using `File.mtime`, `File.stat`, `Integer#zero?`, or `Regexp.last_match(1)` passes CI and passes `ruby -c`, then aborts at converge time on every real target. **CI's `test-linux` / `test-macos` dry-run jobs catch it only where the dry-run actually EXECUTES the line** — they do run the real mruby binary, so a red dry-run job on a green `syntax-check` is the signature of this class, but a green dry-run is NOT evidence of absence. The gap is any code the dry-run cannot reach: `require_external_auth` returns early `unless STDIN.tty?`, and CI has no TTY, so nothing behind that gate (the profile auto-discovery / diagnosis helpers) is ever run by CI. `Regexp.last_match(1)` survived undetected there and was found by hitting the same bug in a private overlay. A stub-DSL render does not catch it either: the stub runs under CRuby, so it is only evidence about interpolation and control flow, never about mruby's API surface. For an unreachable-by-CI path, the detection greps below are the only gate.
