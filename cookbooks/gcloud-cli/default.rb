...
# The only per-OS difference is which release tarball to fetch — download,
# untar, install.sh and the profile fragment are identical on both — so this
# is a value dispatch, not a per-OS split. The archive names stay as the
# pre-migration case arms had them (darwin: Apple Silicon, linux: x86_64).
archive_name = platform_value(
  darwin: "google-cloud-cli-darwin-arm.tar.gz",
  linux: "google-cloud-cli-linux-x86_64.tar.gz",
)
archive_path = "#{installation_home}/#{archive_name}"
