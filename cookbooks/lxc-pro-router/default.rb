...
#
# cookbooks/lxc-pro-router: the pro-router LXC (CT 102) service logic, extracted
# from pve/lxc-pro-router.rb (Phase 4 structural refactor). The thin entry
# recipe includes this cookbook then calls lxc_entry. pro-router is a Tailscale
# subnet router (advertises 192.168.1.0/24, AWS VPC tunnel) — NOT the LAN
# gateway, so a misconfigured apply only breaks the Tailscale advertise path.
...
file "#{node[:setup][:root]}/lxc-pro-router-ip-forward.conf" do
  ...
  mode "644"
  content <<~CFG
    # Managed by setup/pve/lxc-pro-router.rb. Do not edit
...
file "#{node[:setup][:root]}/lxc-pro-router-tailnet-routes.sh" do
  ...
  content <<~SH
    #!/bin/bash
    # Managed by setup/pve/lxc-pro-router.rb. Do not edit by hand.
    #
...
# /host-registry/outputs/* (Phase B-4) so this recipe is decoupled from
