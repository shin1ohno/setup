...
# cron — every 2 min to minimise the detection floor (was 5 min). The observer
# is bash (no LLM cost) and caches the elastic password (PW_CACHE_TTL=1800s) so
# the higher frequency does NOT increase SSM/KMS load. flock -n makes an
# overlapping cycle exit cleanly; `timeout 120` hard-caps a hung cycle. It may
# now occasionally coincide with the auto-mitamae orchestrator (*/5) — harmless,
# since each has its own flock and the SSM read is served from cache.
cron_content = <<~CRON
  # Auto-generated by cookbooks/self-heal-observer. Do not edit.
  PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
  MAILTO=""

  */2 * * * *  root  timeout 120 flock -n /var/lock/self-heal-observer.lock #{OBSERVER_BIN} --once >> #{LOG_FILE} 2>&1
CRON
