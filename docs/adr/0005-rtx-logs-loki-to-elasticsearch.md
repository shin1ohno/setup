...
| 112 (es-0) | ES master+data+ingest | 4 | 8 GB | `/mnt/data/elasticsearch/es-0` (Crucial X8 USB) | 200 GB | -Xms4g -Xmx4g |
| 113 (es-1) | ES master+data+ingest | 4 | 8 GB | `/mnt/data/elasticsearch/es-1` | 200 GB | -Xms4g -Xmx4g |
| 114 (es-2) | ES master+data+ingest | 4 | 8 GB | `/mnt/data/elasticsearch/es-2` | 200 GB | -Xms4g -Xmx4g |
| 115 (kibana) | Kibana | 2 | 4 GB | `/data/kibana` (rpool) | 50 GB | NODE_OPTIONS=--max-old-space-size=1536 |
| **追加合計** |  | **14** | **28 GB** | — | **600 GB on USB X8 + 50 GB rpool** | — |

### Sizing 根拠 (Prometheus 実測 2026-05-08)

| 資源 | 既存 LXC alloc | 新規追加 | 合計 / host 上限 | 消費率 |
|---|---|---|---|---|
| RAM | 34.5 GB | 28 GB | 62.5 / 62.8 GB | **99% (alloc-sum、実使用 ~50%)** |
| vCPU | 25 (overcommit 2.08x) | 14 | 39 / 12 cores | 3.25x overcommit (LXC は idle 主体、現 host CPU 9.8%) |
| Disk rpool | 90 GB | 50 GB | 140 / 928 GB | 15% |
| Disk /mnt/data (X8) | 486 GB | 600 GB | 1086 / 1815 GB | 60% |

Phase 1 着手前に **`sda-backup-20260501.img` (932 GB) を /mnt/data から削除**して 1.3 TB free を確保 (2026-05-08 実施済)。
...
| Phase | Scope | PR 数 | 状態 |
|---|---|---|---|
| 0 | PVE host capacity probe + sda image 削除 | 0 (調査) | **完了 (2026-05-08)** |
| 2 | PVE host capacity Prometheus alert 12 件 (`pve-host.yml`) | 2 (#228, #229) | **完了 (2026-05-08)** |
| 1 | TLS / SSM password / IAM 拡張 (home-monitor terraform) | 1 | 未着手 |
...
- **リソース消費増**: RAM +28 GB (alloc-sum で host 99%、実使用は ~14 GB 増の見込み)、vCPU +14、disk 600 GB on USB X8。同じ host 上に他大物 LXC を追加する余地は事実上消える
