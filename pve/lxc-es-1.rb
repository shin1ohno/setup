...
raise "lxc-es-1: node[:profile][:ip] nil for hostname '#{node[:profile][:hostname]}' — add an es-1 entry to cookbooks/host-profile FLEET" unless es_ip
node.reverse_merge!(
  elasticsearch: {
    node_name: "es-1",
